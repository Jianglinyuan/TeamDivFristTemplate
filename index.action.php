...
class index extends base {
	...
	public function init(){
		...
		$new_shop2=$this->db->GetList("select * from `@#_shoplist` where `sid` in (select id from `@#_shoplist` where `id` = `sid` ORDER BY `id` DESC ) and `q_uid` is null ORDER BY `id` DESC LIMIT 2,12");
		...
		//商品人气排行前8条
		//$hotshop=$this->db->GetList("select sum(gonumber) as gonumbersum,b.* from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id  group by b.sid  ORDER BY gonumbersum DESC LIMIT 8");
		...
		//晒单分享
		$shaidan=$this->db->GetList("select * from `@#_shaidan` a left join `@#_member` b on  a.sd_userid=b.uid where grade!='D'  order by `sd_id` DESC LIMIT 4");
		...
		if($_GET['test']=="test"){
			include templates("index","test");die;
		}
	...
	*/
	...
	public function dataserver(){
		...
		//用户云购时间
		$user_last = $this->db->GetOne("select time,ip from `@#_member_go_record` where `uid`= '$item[q_uid]' and `shopid` = '$itemid' and `shopqishu` = '$item[qishu]' and `huode` = '$item[q_user_code]'");
		$user_shop_time = $user_last['time'];
