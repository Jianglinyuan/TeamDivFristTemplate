...
class tocode {
	...
	public function run_tocode(&$time='',$num=100,$cyrs='233',$apinumber=23498){
		if(empty($time))return false;
		if(empty($num))return false;
		if(empty($cyrs))return false;
		if(empty($apinumber))return false;
		$this->times = $time;
		$this->num = $num;
		$this->cyrs = $cyrs;
	...
	private function get_code_dabai(){
		...
		$this->go_code = 10000001+fmod($count_time+$apinumber,$this->cyrs);			
	}
	...
	private function get_code_yibai(){		
		...
		$num= $h.$i.$s.$w;
		if(!$cyrs)$cyrs=1;
		$this->go_code = 10000001+fmod($num*50+$apinumber,$cyrs);
